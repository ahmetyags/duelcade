...
const EN_MODES: Record<TurnGameMode, TurnModeCopy> = {
  ...
  cipher_clash: {
    title: 'Cipher Clash',
    description: 'Decode the shared hidden rune sequence before your opponent.',
    winReason: 'Decoded the complete secret rune sequence',
    help: {
      goal: 'Both players race to decode the same hidden rune sequence.',
...
const TR_MODES: Record<TurnGameMode, TurnModeCopy> = {
  ...
  cipher_clash: {
    ...
    description: 'Ortak gizli rün dizisini rakibinden önce çöz.',
    winReason: 'Gizli rün dizisinin tamamını çözdü',
    help: {
      goal: 'İki oyuncunun da tahmin ettiği ortak gizli rün dizisini rakibinden önce çöz.',
...
export const TURN_UI = {
  en: {
    ...
    channelTarget: (channel: string, frequency: number) => `Channel ${channel} target: ${frequency} Hz`,
    opponentTurn: "OPPONENT'S TURN",
    skipRound: 'Skip this game', skipTitle: 'Skip this game?',
    skipRemoteTitle: 'Opponent wants to skip',
    skipDescription: 'Your opponent must also approve. No one receives a round point.',
    skipRemoteDescription: 'Approve to move both players to a new random game.',
    skipApprove: 'Request skip', skipAccept: 'Accept', skipDecline: 'Decline',
    skipSoloDescription: 'No one receives a round point.',
    skipSoloApprove: 'Skip game',
    ...
    tutorialFirstMove: 'Tap an empty square to place your first rune.',
    tutorialWatchOpponent: 'Watch DuelBot and block the line it is building.',
    tutorialBuildAndBlock: 'Build your own line before the next bot move.',
  },
  tr: {
    ...
    channelTarget: (channel: string, frequency: number) => `${channel} kanalı hedefi: ${frequency} Hz`,
    opponentTurn: 'RAKİBİN SIRASI',
    skipRound: 'Bu oyunu pas geç', skipTitle: 'Bu oyun pas geçilsin mi?',
    skipRemoteTitle: 'Rakibin pas geçmek istiyor',
    skipDescription: 'Rakibinin de onayı gerekir. Hiç kimse tur puanı almaz.',
    skipRemoteDescription: 'Onaylarsan ikiniz de yeni ve rastgele bir oyuna geçersiniz.',
    skipApprove: 'Pas geçme isteği gönder', skipAccept: 'Onayla', skipDecline: 'Reddet',
    skipSoloDescription: 'Hiç kimse tur puanı almaz.',
    skipSoloApprove: 'Oyunu pas geç',
    ...
    tutorialFirstMove: 'İlk rünü koymak için boş bir kareye dokun.',
    tutorialWatchOpponent: 'DuelBot’un hamlesini izle ve kurduğu çizgiyi engelle.',
    tutorialBuildAndBlock: 'Sıradaki bot hamlesinden önce kendi çizgini kur.',
  },
} as const;
...
export const TURN_RESULTS = {
  ...
  tr: {
    ...
    acceptRematch: 'Rövanşı kabul et',
