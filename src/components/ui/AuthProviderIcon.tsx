...
const decorativeIconAccessibility = Platform.select({
  web: { 'aria-hidden': true as const },
  default: { accessible: false, accessibilityElementsHidden: true },
});

export function AuthProviderIcon({ provider, size = 22 }: { provider: SocialAuthProvider; size?: number }) {
  if (provider === 'google') {
    return (
      <Svg width={size} height={size} viewBox="0 0 24 24" {...decorativeIconAccessibility}>
